...
export default function HabitSettings({onClose, updateOrCreate, existingHabit}: SettingsProps) {
  ...
  return (
    <form className="settings" onSubmit={handleSubmit}>
        <div className="form-fields">
            ...
            <input name='name' type='text' defaultValue={existingHabit?.name || ''} className={nameError ? "error" : ''}></input>
            <label htmlFor='color'>
                Цвет
            </label>
            <input name='color' type='text' defaultValue={existingHabit?.color || ''}></input>
            <label htmlFor='maxSize'>
                Размер
            </label>
            <input name='maxSize' type='text' defaultValue={existingHabit?.size || ''}></input>
        </div>
        <div style={{display: 'flex', gap: '5px'}}>
            <button onClick={() => onClose()}>Cancel</button>
            <button type="submit">{existingHabit ? 'Update' : 'Create'}</button>
        </div>
    </form>
  )
}
