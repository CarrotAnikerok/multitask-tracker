...
export const App = ({ initialHabits, onChange }: AppProps) => {
    ...
    const updateHabits = (habitToUpdate: Habit) => {
        ...
        if (isHabitExist) {
            setHabits(prev => 
                prev.map(h => 
                    h.id === habitToUpdate.id ? habitToUpdate : h
                )
            );
        } else {
    ...
    return (
        <div>
            <div className="container" ref={containerRef}>
                {isHabitCreation 
                ? <HabitSettings onClose={() => setHabitCreation(false)} updateOrCreate={updateHabits}></HabitSettings>
                : <div className="setting-buttons">
                    <SortPanel setHabits={setHabits}></SortPanel>
                    <button onClick={() => setHabitCreation(true)}>+</button>
                  </div>}
            {habits.map((habit) => {
                return <HabitBlock key={habit.id} habit={habit} handleUpdate={updateHabits} handleDelete={deleteHabit}></HabitBlock>;
            }
            )}
            </div>
        </div>
